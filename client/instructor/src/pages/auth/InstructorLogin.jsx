  ...
  const onSubmit = async (e) => {
    ...
    if (loginUser.fulfilled.match(result)) {
      ...
      if (user?.role !== "instructor") {
        toast.error("Only instructor can login here");
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-xl p-6 md:p-8">
        ...
        <p className="text-sm text-slate-500 mt-1">Use your instructor credentials</p>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-slate-900 text-white rounded-md py-2.5 font-medium hover:bg-slate-800 disabled:opacity-60"
          >
