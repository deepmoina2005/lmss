...
import { addCourse } from "../../redux/slice/educatorSlice";
...
const AddCourse = () => {
  ...
  const addLecture = () => {
    if (!lectureDetails.lectureVideo) {
      toast.error("Please upload lecture video");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const cleanedCourseContent = chapters.map((chapter) => ({
      ...
      chapters.forEach((chapter) => {
        chapter.chapterContent.forEach((lecture) => {
          if (lecture.lectureVideo) {
            formData.append(`lecture_video_${lecture.lectureId}`, lecture.lectureVideo);
          }
        });
      });

      const result = await dispatch(addCourse(formData));
      if (addCourse.rejected.match(result)) {
        toast.error(result.payload || "Failed to add course");
        return;
      }
    ...
    <>
      <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md w-full text-gray-500">
                  ...
                  <div className="p-4">
                    {chapter.chapterContent.map((lecture, lectureIndex) => (
                      <div key={lectureIndex} className="flex justify-between items-center mb-2">
                        <span>
                          {lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins - {lecture.lectureVideo?.name || "Video Uploaded"} - {lecture.isPreviewFree ? "Free Preview" : "Paid"}
                        </span>
                        <img
                ...
                <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                  ...
                  <div className="mb-2">
                    ...
                    <input
                      ...
                      accept="video/*"
                      className="mt-1 block w-full border rounded py-1 px-2"
                      onChange={(e) => setLectureDetails({ ...lectureDetails, lectureVideo: e.target.files?.[0] || null })}
                    />
                  </div>
                  ...
                  <button className="w-full bg-blue-400 text-white rounded py-2" onClick={addLecture} type="button">
                    Add
                  </button>
          ...
          <button type="submit" className="bg-black font-semibold text-white w-max pt-2.5 px-8 py-2 rounded my-4">
            ADD
          </button>
        </form>
      </div>    </>
