...
const StudentRegister = () => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const result = await dispatch(registerUser({ ...form, role: "student" }));
    if (registerUser.fulfilled.match(result)) {
      toast.success(result.payload?.message || "Student registration successful");
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-xl p-6 md:p-8">
        ...
        <p className="text-sm text-slate-500 mt-1">Create a student account</p>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <div><label className="text-sm font-medium text-slate-700">Email</label><input name="email" type="email" value={form.email} onChange={onChange} required className="mt-1 w-full border border-slate-300 rounded-md px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800" placeholder="student@example.com" /></div>
        ...
        <p className="mt-4 text-sm text-slate-600">Already have account? <Link to="/auth/student/login" className="text-blue-600 hover:underline">Student login</Link></p>
      </div>
    </div>
  );
};
