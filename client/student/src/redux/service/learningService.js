...
export const submitCourseDoubtApi = async (token, payload) => {
  const { data } = await api.post("/api/user/doubts", payload, authHeaders(token));
  return data;
};

export const fetchCourseDoubtsApi = async (token, courseId) => {
  const { data } = await api.get(`/api/user/doubts/${courseId}`, authHeaders(token));
  return data;
};
